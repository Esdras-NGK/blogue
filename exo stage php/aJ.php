...
// $jeu = $connection->query('SELECT * FROM jeu ORDER BY id DESC');


if(isset($_POST['id']) AND !empty($_POST['id'])) {
   $get_id = strip_tags($_POST['id']);
   $article = $connection->prepare('SELECT * FROM jeu WHERE id = ?');
            ...
            <section class="col-8 p-4" id="section" style="display: flex;flex-direction: row;flex-wrap: wrap;">
                ...
                <?php while ($j = $jeu->fetch()) { ?>
                    ...
                    <div class="card-body">
                            ...
                            <p class="card-text"><?= $j['contenu'] ?></p>
                            |<a class="btn btn-outline-secondary" href="jeu.php?edit=<?= $j['id'] ?>">modifier</a> | <a class="btn btn-outline-secondary" href="supprime.php?id=<?= $j['id'] ?>">supprimer</a>             
                        </div>
                    </div>
                <?php } ?>
